...
contract PirateContract is IERC721, Ownable {

  uint256 public constant gen0CreationLimit = 5;

  string private constant nameOfToken = "Pirates";
  ...
  Pirate[] private pirates;

  mapping(uint256 => address) private pirateOwners;
  ...
  constructor() {
    _createPirate(type(uint256).max, 0, 0, 0, address(0));
  }
  ...
  function getPirate(uint256 _tokenId) external view returns (
    uint256 genes,
    uint256 birthTime,
    uint256 dadId,
    uint256 momId,
    uint256 generation,
    address owner) {
      require(_tokenId < pirates.length, "Token does not exist");
      return (
        pirates[_tokenId].genes,
        pirates[_tokenId].birthTime,
        pirates[_tokenId].dadId,
        pirates[_tokenId].momId,
        pirates[_tokenId].generation,
        pirateOwners[_tokenId]
      );
    }

    function allOwnedPirates() external view returns(uint256[] memory) {
        uint256[] memory ownedPirates = new uint256[](tokenBalances[msg.sender]);
        uint256 counter = 0;
        for(uint256 i = 0; i < pirates.length; i++) {
          if(pirateOwners[i] == msg.sender) {
            ownedPirates[counter] = i;
            counter++;
          }
        }
        return ownedPirates;
  ...
  function _createPirate(
    ...
    address _owner
      ...
      Pirate memory _pirate = Pirate(_genes, uint64(block.timestamp), uint32(_dadId), uint32(_momId), uint16(_generation));

      pirates.push(_pirate);
      uint256 newPirateId = pirates.length - 1;

      emit Birth(_owner, newPirateId, _genes, _momId, _dadId);

      _transfer(address(0), _owner, newPirateId);
  ...
  function approve(address _approved, uint256 _tokenId) override external {
    require(_owns(msg.sender, _tokenId) || operatorApproval[pirateOwners[_tokenId]][msg.sender], "You are not the owner or the operator of this token");
  ...
  function _owns(address _claimant, uint256 _tokenId) public view returns (bool) {
    return _claimant == pirateOwners[_tokenId];
  }
}
